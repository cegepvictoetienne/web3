...
Dans certaines situations, le BrowserRouter cause des problèmes. Par exemple, déployer un site avec BrowserRouter sans pouvoir changer les directives du serveur Web, rend inutilisable le site car BrowserRouter essaie de lire les pages du site qui n'existent pas au niveau du serveur Web. (Tout se passe dans index.html). 

Le remède est l'utilisation de HashRouter, qui utilise un `#` dans l'url, ce qui force le navigateur à rester dans le même index.html.  
...
Dans l'exemple ci-haut, l'url de la page de l'auteur serait : http://localhost:3000/#/dadams pour le HashRouter.  
